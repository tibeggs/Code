...
for filename in filelists:
    ...
    df['clean_flag']=0
    df.loc[(df['ap'].isna()), 'clean_flag']=1
    df.loc[(df['ap']<0), 'clean_flag']=1
    ...
    df2.loc[(df2['ap']==0)&df2['e_range'].isna(),'e_range']=0
    df2.loc[df2['e_range']<0,'e_range']=0  
    #df2.sort_values(['stctyid','naics_len'],ascending=False)
    lister=df2.loc[(df2['ap']>0)&(df2['naics_len']>2),'cty_naics'].values
    
    for i in reversed(lister):
        code=i[:-1]
        cu=df2.loc[df2['cty_naics']==i,'ap'].item()
        try:
            up=df2.loc[df2['cty_naics']==code,'ap'].item()
        except:
            up=0
        if up < cu:
            org=df2.loc[df2['cty_naics']==code,'ap']
            df2.loc[df2['cty_naics']==code,'ap']=org+(df2.loc[df2['cty_naics']==i, 'ap'].item())   
    df3= df.loc[~df['stctyid'].isin(df1['stctyid'])]
    df3.to_csv(cleanpath+"/ap_sucess_"+filename)
    cbp_erange=df2
...
#    cbp_erange.loc[cbp_erange['naics2'].isin(['49']),'naics2'] = '48'
    ...
    for naics_code in cbp_erange[cbp_erange['naics_len']==2].index:
        if (cbp_erange.loc[naics_code,'naics2']!='99')&(cbp_erange.loc[naics_code,'naics2']!='To'):
            dfl=cbp_erange[['stctyid','naics_len','ap','e_range']].groupby(['stctyid','naics_len']).sum().reset_index()
            dfl.columns = ['stctyid','naics_len','ap_sum','erange_sum']
            dfl1=cbp_erange[['stctyid','naics2','naics_len','ap','e_range']].groupby(['stctyid','naics2','naics_len']).sum().reset_index()
            dfl1.columns = ['stctyid','naics2','naics_len','ap_sum','erange_sum']
            if cbp_erange.loc[naics_code,'ap']==0:
                a = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))& (dfl['naics_len']==2), 'ap_sum'].item()
                b = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))& (dfl['naics_len']==1), 'ap_sum'].item()
                dif = b-a
                ...
                per = c/d
                e = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics2']==(cbp_erange.loc[(naics_code),'naics2']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+1)), 'ap_sum'].item()
                ...
                try:
                    g = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics2']==(cbp_erange.loc[(naics_code),'naics2']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+2)), 'ap_sum'].item()
                except:
                    g = 0
                if (dif!=0)&((per*dif)-e>0)&((per*dif)-e<g):
                    e=e+(g-((per*dif)-e))
                try:
                    h = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics2']==(cbp_erange.loc[(naics_code),'naics2']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+3)), 'ap_sum'].item()
                except:
                    h = 0
                if (dif!=0)&((per*dif)-e>0)&((per*dif)-e<h):
                    e=e+(h-((per*dif)-e))
                try:
                    i = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics2']==(cbp_erange.loc[(naics_code),'naics2']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+4)), 'ap_sum'].item()
                except:
                    i = 0
    ...
    for State_var in [1004]:
        ...
        if count == 1:
            if cbp_erange.loc[count1_string,'ap']==0:
            ...
            for NAICS_var in [48]:
                if str(State_var)+"-"+str(NAICS_var) in cbp_erange.index:
                    if pd.notna(cbp_erange.loc[str(State_var)+'-'+str(NAICS_var),'e_range']):
                        ...
                        if new > cbp_erange.loc[str(State_var)+'-'+str(NAICS_var),'ap'].item():
                            cbp_erange.loc[str(State_var)+'-'+str(NAICS_var),'ap'] = cbp_erange.loc[str(State_var)+'-'+str(NAICS_var),'e_range'] / flagsum * NAICS_rem
    
    t22 = time.time()
    print(t22-t21)
  
    for naics_code in cbp_erange[cbp_erange['naics_len']==3].index:
        dfl=cbp_erange[['stctyid','naics2','naics_len','ap','e_range']].groupby(['stctyid','naics2','naics_len']).sum().reset_index()
        dfl.columns = ['stctyid','naics2','naics_len','ap_sum','erange_sum']
        dfl1=cbp_erange[['stctyid','naics3','naics_len','ap','e_range']].groupby(['stctyid','naics3','naics_len']).sum().reset_index()
        dfl1.columns = ['stctyid','naics3','naics_len','ap_sum','erange_sum']
        if cbp_erange.loc[naics_code,'ap']==0:
          #  dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics2']==(cbp_erange.loc[(naics_code),'naics2']))]
            a = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics2']==(cbp_erange.loc[(naics_code),'naics2'])) & (dfl['naics_len']==3), 'ap_sum'].item()
            b = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics2']==(cbp_erange.loc[(naics_code),'naics2']))& (dfl['naics_len']==2), 'ap_sum'].item()
            dif = b-a
            c = cbp_erange.loc[naics_code,'e_range']
            ...
            per = c/d
            e = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics3']==(cbp_erange.loc[(naics_code),'naics3']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+1)), 'ap_sum'].item()
            ...
            try:
                g = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics3']==(cbp_erange.loc[(naics_code),'naics3']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+2)), 'ap_sum'].item()
            except:
                g = 0
            if (dif!=0)&((per*dif)-e>0)&((per*dif)-e<g):
                e=e+(g-((per*dif)-e))
                ...
                for NAICS_var_2 in range(10):
                    if str(State_var)+"-"+str(NAICS_var)+str(NAICS_var_2) in cbp_erange.index:
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+str(NAICS_var)+str(NAICS_var_2),'e_range']):
                            ...
                            if new >  cbp_erange.loc[str(State_var)+'-'+str(NAICS_var)+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+str(NAICS_var)+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+str(NAICS_var)+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem 
                ...
                for NAICS_var_2 in range(10):
                    if str(State_var)+"-31"+str(NAICS_var_2) in cbp_erange.index: 
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'31'+str(NAICS_var_2),'e_range']):
                            ...
                            if new > cbp_erange.loc[str(State_var)+'-'+'31'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'31'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'31'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
    
                    if str(State_var)+"-32"+str(NAICS_var_2) in cbp_erange.index: 
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'32'+str(NAICS_var_2),'e_range']):
                            new = cbp_erange.loc[str(State_var)+'-'+'32'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                            if new >cbp_erange.loc[str(State_var)+'-'+'32'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'32'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'32'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
      
                    if str(State_var)+"-33"+str(NAICS_var_2) in cbp_erange.index:  
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'33'+str(NAICS_var_2),'e_range']):
                            new = cbp_erange.loc[str(State_var)+'-'+'33'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                            if new >cbp_erange.loc[str(State_var)+'-'+'33'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'33'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'33'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                ...
                for NAICS_var_2 in range(10):
                    if str(State_var)+"-44"+str(NAICS_var_2) in cbp_erange.index: 
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'44'+str(NAICS_var_2),'e_range']):
                            ...
                            if new > cbp_erange.loc[str(State_var)+'-'+'44'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'44'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'44'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                    if str(State_var)+"-45"+str(NAICS_var_2) in cbp_erange.index:  
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'45'+str(NAICS_var_2),'e_range']):
                            new = cbp_erange.loc[str(State_var)+'-'+'45'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                            if new> cbp_erange.loc[str(State_var)+'-'+'45'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'45'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'45'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem   
                ...
                for NAICS_var_2 in range(10):
                    if str(State_var)+"-48"+str(NAICS_var_2) in cbp_erange.index:
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'48'+str(NAICS_var_2),'e_range']):
                            ...
                            if new > cbp_erange.loc[str(State_var)+'-'+'48'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'48'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'48'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                    if str(State_var)+"-49"+str(NAICS_var_2) in cbp_erange.index: 
                        if pd.notna(cbp_erange.loc[str(State_var)+'-'+'49'+str(NAICS_var_2),'e_range']):
                            new = cbp_erange.loc[str(State_var)+'-'+'49'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
                            if new > cbp_erange.loc[str(State_var)+'-'+'49'+str(NAICS_var_2),'ap'].item():
                                cbp_erange.loc[str(State_var)+'-'+'49'+str(NAICS_var_2),'ap'] = cbp_erange.loc[str(State_var)+'-'+'49'+str(NAICS_var_2),'e_range'] / flagsum * NAICS_rem
    ...
    for naics_code in cbp_erange[cbp_erange['naics_len']==4].index:
        dfl=cbp_erange[['stctyid','naics3','naics_len','ap','e_range']].groupby(['stctyid','naics3','naics_len']).sum().reset_index()
        dfl.columns = ['stctyid','naics3','naics_len','ap_sum','erange_sum']
        dfl1=cbp_erange[['stctyid','naics4','naics_len','ap','e_range']].groupby(['stctyid','naics4','naics_len']).sum().reset_index()
        dfl1.columns = ['stctyid','naics4','naics_len','ap_sum','erange_sum']
        if cbp_erange.loc[naics_code,'ap']==0:
            a = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics3']==(cbp_erange.loc[(naics_code),'naics3'])) & (dfl['naics_len']==4), 'ap_sum'].item()
            b = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics3']==(cbp_erange.loc[(naics_code),'naics3']))& (dfl['naics_len']==3), 'ap_sum'].item()
            dif = b-a
            c = cbp_erange.loc[naics_code,'e_range']
            ...
            per = c/d
            e = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics4']==(cbp_erange.loc[(naics_code),'naics4']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+1)), 'ap_sum'].item()
            ...
            try:
                g = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics4']==(cbp_erange.loc[(naics_code),'naics4']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+2)), 'ap_sum'].item()
            except:
                g=0
            if (dif!=0)&((per*dif)-e>0)&((per*dif)-e<g):
                e=e+(g-((per*dif)-e))
    ...
    for naics_code in cbp_erange[cbp_erange['naics_len']==5].index:
        dfl=cbp_erange[['stctyid','naics4','naics_len','ap','e_range']].groupby(['stctyid','naics4','naics_len']).sum().reset_index()
        dfl.columns = ['stctyid','naics4','naics_len','ap_sum','erange_sum']
        dfl1=cbp_erange[['stctyid','naics5','naics_len','ap','e_range']].groupby(['stctyid','naics5','naics_len']).sum().reset_index()
        dfl1.columns = ['stctyid','naics5','naics_len','ap_sum','erange_sum']
        if cbp_erange.loc[naics_code,'ap']==0:
            a = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics4']==(cbp_erange.loc[(naics_code),'naics4'])) & (dfl['naics_len']==5), 'ap_sum'].item()
            b = dfl.loc[(dfl['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl['naics4']==(cbp_erange.loc[(naics_code),'naics4']))& (dfl['naics_len']==4), 'ap_sum'].item()
            dif = b-a
            c = cbp_erange.loc[naics_code,'e_range']
            ...
            per = c/d
            e = dfl1.loc[(dfl1['stctyid']==(cbp_erange.loc[(naics_code),'stctyid']))&(dfl1['naics5']==(cbp_erange.loc[(naics_code),'naics5']))&(dfl1['naics_len']==(cbp_erange.loc[(naics_code),'naics_len']+1)), 'ap_sum'].item()
    ...
    print(total_time4)
     #Export filled data
    cbp_erange.to_csv(cleanpath+"/ap_try3_"+filename)